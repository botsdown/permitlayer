...
fn happy_path_writes_managed_layer_empty_operator_and_boots() {
    ...
    // The managed bundle ships 9 policies: 2 retained originals
    // (gmail-read-only, drive-research-scope-restricted) plus the 7
    // per-service tier templates (gmail/calendar read-only + read-write,
    // Drive read-only + read-write + full-control, incl. the
    // gmail-read-only-tier alias). The legacy
    ...
    // exactly 9 — asserting the daemon actually compiled the managed
