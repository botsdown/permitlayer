...
pub async fn run(args: StartArgs) -> Result<(), StartError> {
    ...
    // One-time upgrade cleanup for ADR-0008 installations. Those files were
    // explicitly transient, but removing the periodic sweeper without this
    // migration could leave a recently downloaded attachment behind forever.
    // New attachment delivery never creates this directory.
    match cleanup_legacy_media_dir(&config.paths.home.join("media")) {
        Ok(true) => tracing::info!("removed legacy attachment media directory"),
        Ok(false) => {}
        Err(error) => tracing::warn!(%error, "could not remove legacy attachment media directory"),
    }
...
/// Remove the transient attachment store used before ADR-0009. A symlink at
/// the legacy path is unlinked rather than followed; unexpected non-directory
/// files are left untouched.
fn cleanup_legacy_media_dir(path: &std::path::Path) -> std::io::Result<bool> {
...
mod tests {
    ...
    #[test]
    fn cleanup_legacy_media_dir_removes_transient_tree() {
        let tmp = tempfile::tempdir().unwrap();
        let media = tmp.path().join("media");
        let nested = media.join("agent").join("nonce");
        std::fs::create_dir_all(&nested).unwrap();
        std::fs::write(nested.join("old.pdf"), b"transient").unwrap();

        assert!(cleanup_legacy_media_dir(&media).unwrap());
        assert!(!media.exists());
        assert!(!cleanup_legacy_media_dir(&media).unwrap());
    }
